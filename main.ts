...
function sendArrow(direction: string) {
    ...
    arrowSprite = sprites.create(img`
        ...
        `, SpriteKind.Arrow)
    if (direction == "left") {
        arrowSprite.setImage(assets.image`
            leftArrow
            `)
        arrowSprite.x = 20
...
controller.up.onEvent(ControllerButtonEvent.Pressed, function on_up_pressed() {
    //ends if length is less than or equal to zero
    if (upArrow.length <= 0) {
        return
    }
    // inital values
    ...
    // makes sure that arrows in array is removed
    if (targetArrow !== null && targetIndex !== -1) {
        // inverts sprite image
        targetArrow.setImage(assets.image`upArrow-B`)
    ...
    //  animation
    scoreboard2.setImage(assets.image`
        scoreboardBackground
        `)
    scoreboard2.setImage(assets.image`
        sc2animated
...
controller.left.onEvent(ControllerButtonEvent.Pressed, function on_left_pressed() {
    ...
    info.changeScoreBy(100 / (1 + Math.abs((80 - lowestYValue) / easynessToGetPerfect)))
    scoreboard1.setImage(assets.image`
        scoreboardBackground
        `)
    scoreboard1.setImage(assets.image`
        sc1animated
...
controller.right.onEvent(ControllerButtonEvent.Pressed, function on_right_pressed() {
    ...
    info.changeScoreBy(100 / (1 + Math.abs((80 - lowestYValue) / easynessToGetPerfect)))
    scoreboard4.setImage(assets.image`
        scoreboardBackground
        `)
    scoreboard4.setImage(assets.image`
        sc4animated
...
controller.down.onEvent(ControllerButtonEvent.Pressed, function on_down_pressed() {
    ...
    if (targetArrow !== null && targetIndex !== -1) {
        targetArrow.setImage(assets.image`downArrow`)
        downArrow.removeAt(targetIndex)
    }

    info.changeScoreBy(100 / (1 + Math.abs((80 - lowestYValue) / easynessToGetPerfect)))
    scoreboard3.setImage(assets.image`
        scoreboardBackground
        `)
    scoreboard3.setImage(assets.image`
        sc3animated
